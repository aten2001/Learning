...
var HousingScatterPlotMatrixCanvas = Backbone.View.extend({
  initialize: function(args) {
    ...
    this.labels
      .text(function(d) { return DIMENSION_LONGFORM[d.id]; })
      .attr('font-size', 10)
      .attr('fill', '#555')
...
var SplitHistogramComparisonView = Backbone.View.extend({
  initialize: function(args) {
    ...
    var rightSideAttribute = DIMENSION_LONGFORM[tree_stats[gtNodeID].id];
    ...
    var leftSideAttribute = DIMENSION_LONGFORM[tree_stats[ltNodeID].id];
    ...
    this.labels = this.g.selectAll(".label")
      ...
      .enter()
        ...
        .text(function(d) {
          return DIMENSION_LONGFORM[d.id];
        });
...
var SplitIndicator = Backbone.View.extend({
  ...
  render: function() {
    var displayNumber = Math.round(this.current.split);
    this.textNode.text(displayNumber);
...
var TreeNodeView = Backbone.View.extend({
  ...
  render: function() {
    ...
    this.label
      .text(DIMENSION_LONGFORM[this.stats.attribute])
      .attr("opacity", this.histogramOpacity);
