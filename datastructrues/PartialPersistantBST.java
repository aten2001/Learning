...
public class PartialPersistantBST<Key,Value> {

  private static final int N = 10; // number of versions
  class Node {
    Key key;
    Value[] value; //value per version
    Node left, right, old;
    int count;
    int version;
    ...
    public Value  getVal(int version){
      if(version > this.version) throw new RunTimeException("version exceeds range");
      int nodeCount = version / this.value.length;
      int currentNode = this.version/ this.value.length; 
      Node n = this;
      while(nodeCount < currentNode){
        n = this.old;
        currentNode--;
      }
      return n.value[this.version % this.value.length];
    }
